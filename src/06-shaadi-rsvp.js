...
export function transformGuestList(guests, ...transformFns) {
  ...
  // Reducing the functions array to apply them sequentially left-to-right
  return transformFns.reduce((acc, currentFn) => {
    return typeof currentFn === "function" ? currentFn(acc) : acc;
  }, guests);
}